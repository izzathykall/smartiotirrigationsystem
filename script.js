    ...
    function updateHistoryWindow() {
      ...
      if (info) {
        ...
        info.innerHTML = `Showing data ${start} - ${end} of ${allHistoryData.length}<br>${firstDate} → ${lastDate}`;
      }
    }
    ...
    function renderGraph(field) {
      ...
      historyChart = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            legend: { labels: { color: cssVar("--chart-text"), font: { weight: "bold" } } },
            tooltip: {
              padding: 12,
              displayColors: false,
          ...
          scales: {
            x: {
              ticks: { color: cssVar("--chart-text"), maxRotation: 45, minRotation: 0 },
    ...
    function getTankFillPercent(level) {
    ...
    function updateTankVisual(fillId, noteId, level) {
      const fillEl = document.getElementById(fillId);
      const noteEl = document.getElementById(noteId);
      if (!fillEl || !noteEl) return;

      const normalized = String(level || "--").toUpperCase();
      const pct = getTankFillPercent(normalized);
    ...
    onValue(latestRef, (snapshot) => {
      ...
      setAnimatedText("borewell", data.borewellLevel ?? "--");
      setAnimatedText("spareTank", data.spareTankLevel ?? "--");
      updateTankVisual("borewellTankFill", "borewellTankNote", data.borewellLevel ?? "--");
      updateTankVisual("spareTankFill", "spareTankNote", data.spareTankLevel ?? "--");
